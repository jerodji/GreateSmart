...
@interface SYATopScrollViewController ()<UICollectionViewDataSource,UICollectionViewDelegate>
{
    UIColor *_norColor;
    UIColor *_selColor;
}
...
- (UIScrollView *)contentScrollView
{
    if (_contentScrollView == nil) {
        ...
        // 创建布局
        SYAFlowLayout *layout = [[SYAFlowLayout alloc] init];
...
- (void)setIsShowTitleScale:(BOOL)isShowTitleScale
{
    if (_isShowUnderLine) {
        // 抛异常
        NSException *excp = [NSException exceptionWithName:@"SYATopScrollViewControllerException" reason:@"字体放大效果和角标不能同时使用。" userInfo:nil];
        [excp raise];
    }
...
- (void)setIsfullScreen:(BOOL)isfullScreen
{
    ...
    self.contentView.frame = CGRectMake(0, 0, SYAScreenW, SYAScreenH);
...
- (void)setUpTitleGradient:(void(^)(kTitleColorStyle *titleColorGradientStyle,UIColor **norColor,UIColor **selColor))titleGradientBlock;
{
    ...
    if (_titleColorGradientStyle == kTitleColorStyleFill && _titleWidth > 0) {
        @throw [NSException exceptionWithName:@"SYA_ERROR" reason:@"标题颜色填充不需要设置标题宽度" userInfo:nil];
    }
}
...
- (void)viewDidLayoutSubviews
{
    ...
    if (_isInitial == NO) {
        ...
        if (_isfullScreen) {
            ...
            // 顶部标题View尺寸
            self.titleScrollView.frame = CGRectMake(0, titleY, SYAScreenW, self.titleHeight);
        ...
        if (self.contentView.frame.size.height == 0) {
            self.contentView.frame = CGRectMake(0, titleY, SYAScreenW, SYAScreenH - titleY);
        }
        
        // 顶部标题View尺寸
        self.titleScrollView.frame = CGRectMake(0, 0, SYAScreenW, self.titleHeight);
...
- (void)setUpTitleWidth
{
    ...
    CGFloat titleMargin = (SYAScreenW - totalWidth) / (count + 1);
...
- (void)setUpAllTitle
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        UILabel *label = [[SYATopScrollViewTitleLabel alloc] init];
...
// 设置标题颜色渐变
- (void)setUpTitleColorGradientWithOffset:(CGFloat)offsetX rightLabel:(SYATopScrollViewTitleLabel *)rightLabel leftLabel:(SYATopScrollViewTitleLabel *)leftLabel
{
    if (_isShowTitleGradient == NO) return;
    
    // 获取右边缩放
    CGFloat rightSacle = offsetX / SYAScreenW - leftLabel.tag;
    
    // 获取左边缩放比例
    CGFloat leftScale = 1 - rightSacle;
    
    // RGB渐变
    if (_titleColorGradientStyle == kTitleColorStyleRGB) {
    ...
    // 填充渐变
    if (_titleColorGradientStyle == kTitleColorStyleFill) {
        ...
        if (offsetDelta > 0) { // 往右边
            ...
            rightLabel.scrollProgress = rightSacle;
            
            leftLabel.textColor = self.selColor;
            leftLabel.fillColor = self.norColor;
            leftLabel.scrollProgress = rightSacle;
...
- (void)setUpUnderLineOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel
{
    ...
    // 获取两个标题中心点距离
    CGFloat centerDelta = rightLabel.sya_x - leftLabel.sya_x;
    ...
    // 计算当前下划线偏移量
    CGFloat underLineTransformX = offsetDelta * centerDelta / SYAScreenW;
    
    // 宽度递增偏移量
    CGFloat underLineWidth = offsetDelta * widthDelta / SYAScreenW;
...
- (void)setUpCoverOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel
{
    ...
    // 计算当前下划线偏移量
    CGFloat coverTransformX = offsetDelta * centerDelta / SYAScreenW;
    
    // 宽度递增偏移量
    CGFloat coverWidth = offsetDelta * widthDelta / SYAScreenW;
...
- (void)titleClick:(UITapGestureRecognizer *)tap
{
    ...
    if (vc.view) {
        // 发出通知点击标题通知
        [[NSNotificationCenter defaultCenter] postNotificationName:SYATopTitleClickOrScrollDidFinshNote  object:vc];
        
        // 发出重复点击标题通知
        if (_selIndex == i) {
            [[NSNotificationCenter defaultCenter] postNotificationName:SYATopTitleRepeatClickTitleNote object:vc];
        }
    }
...
- (void)setUpUnderLine:(UILabel *)label
{
    ...
    CGFloat underLineH = _underLineH?_underLineH:SYAUnderLineH;
    
    self.underLine.sya_y = label.sya_height - underLineH;
    self.underLine.sya_height = underLineH;
    
    
    // 最开始不需要动画
    if (self.underLine.sya_x == 0) {
        if (_isUnderLineEqualTitleWidth) {
            self.underLine.sya_width = titleBounds.size.width;
        } else {
            self.underLine.sya_width = label.sya_width;
        }
...
- (void)setLabelTitleCenter:(UILabel *)label
{
    ...
    // 计算下最大的标题视图滚动区域
    CGFloat maxOffsetX = self.titleScrollView.contentSize.width - SYAScreenW + _titleMargin;
...
- (void)refreshDisplay
{
    if (self.childViewControllers.count == 0) {
        @throw [NSException exceptionWithName:@"SYA_ERROR" reason:@"请确定添加了所有子控制器" userInfo:nil];
    }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    NSInteger screenWInt = SYAScreenW;
    
    NSInteger extre = offsetXInt % screenWInt;
    if (extre > SYAScreenW * 0.5) {
        // 往右边移动
        offsetX = offsetX + (SYAScreenW - extre);
        _isAniming = YES;
        [self.contentScrollView setContentOffset:CGPointMake(offsetX, 0) animated:YES];
    }else if (extre < SYAScreenW * 0.5 && extre > 0){
        _isAniming = YES;
        // 往左边移动
    ...
    // 获取角标
    NSInteger i = offsetX / SYAScreenW;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    // 获取左边角标
    NSInteger leftIndex = offsetX / SYAScreenW;
    
    // 左边按钮
    SYATopScrollViewTitleLabel *leftLabel = self.titleLabels[leftIndex];
    
    // 右边角标
    NSInteger rightIndex = leftIndex + 1;
    
    // 右边按钮
    SYATopScrollViewTitleLabel *rightLabel = nil;
