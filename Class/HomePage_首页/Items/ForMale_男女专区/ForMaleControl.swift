...
//    }
    ...
    func handleData(typeInfo:Any!) -> ForMaleModel {
        ...
        view.leftImgView.sd_setImage(with: URL.init(string: (model.data.block[0] as! ForMaleBlockElemModel).imageUrl), completed: nil)
        view.rightImgView.sd_setImage(with: URL.init(string: (model.data.block[1] as! ForMaleBlockElemModel).imageUrl), completed: nil)
        return model
    }
